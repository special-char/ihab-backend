...
export class OrdersService {
  ...
  async findAll(query?: any): Promise<Order[]> {
    ...
    if (query?.retailerId) {
      const retailerIds = await this.retailerOfferModel
        .find({ retailer: query?.retailerId })
        .select('_id');
      searchQuery = {
        ...searchQuery,
        retailerOffer: {
  ...
  async downlaodOrders() {
    ...
    worksheet.columns = [
      ...
      {
        header: 'Retailer Payment Status',
        key: 'retailerPaymentStatus',
        width: 20,
      },
    ...
    const File = await new Promise((resolve) => {
      tmp.file(
        { discardDescriptor: true, prefix: 'adminList', postfix: '.xlsx' },
